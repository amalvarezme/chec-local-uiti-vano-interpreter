...
Desarrollo de modelos de Inteligencia Artificial (predictiva, generativa y agéntica) orientados al análisis de fallas en redes de distribución eléctrica de Nivel de Tensión 2. El objetivo es evaluar el impacto de estas fallas mediante UiTi y su ponderación por vano (`UITI_VANO`). En la versión actual del flujo MGCECDL, el entregable predictivo se concentra en **clasificación de impacto**: `UITI_VANO` se usa como objetivo o para derivar clases ordinales, pero no entra como predictor del modelo.
...
3. Plantear y evaluar escenarios de simulación "*What-If*" (qué pasa si), alterando valores en las características para estimar mediante IA cambios en la clase o nivel de impacto asociado a `UITI_VANO`.
...
| **UITI_VANO** | UiTi de la falla ponderado por el aporte relativo del vano. Es objetivo de entrenamiento o variable base para discretizar la clase de impacto; debe excluirse de `features` para evitar fuga de información. |
...
**Regla de entrenamiento MGCECDL:** aunque `UITI_VANO` pueda aparecer en el archivo de selección de variables, el preprocesamiento lo separa como objetivo y no lo incluye en `X`. En consecuencia, un agente puede usar `UITI_VANO`, `UITI_VANO_PROM` o clases derivadas para priorizar, evaluar o narrar impacto, pero no debe presentarlo como variable explicativa usada por el clasificador.
...
2. **Grafo estimado por muestra o escenario:** entregable interpretativo generado en su día por `05_mgcecdl_circuit_analysis.ipynb` (borrado del árbol el 2026-08-14; sigue en el historial de git). Se estima a partir de la capa de reconstrucción del modelo MGCECDL y una similitud RBF entre perfiles reconstruidos de variables. Se exporta como HTML interactivo en `reports/mgcecdl-results/interactive_graphs/`.
