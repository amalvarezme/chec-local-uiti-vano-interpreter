...
**`06` is interactive-only and has no artifact.** Unlike `02`/`03`/`04`, it renders no standalone HTML: its panel is `ipywidgets` driven from a live kernel, so there is nothing for an app-style command to serve and no `/app-*` sibling for it. Uploading it is still worth it — it runs in the Databricks UI attached to the right compute — but set expectations accordingly when reporting back.
