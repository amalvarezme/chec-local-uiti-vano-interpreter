...
Still forbidden: MUST NOT modify any file under `notebooks/` (the shimmed copies are always staged copies), and MUST NOT create any `site`-named path inside the Volume — the project's `site/` tree is only ever regenerated by a local run.
