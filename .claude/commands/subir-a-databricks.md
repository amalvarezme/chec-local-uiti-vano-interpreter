...
**Out of scope**: this command MUST NOT provision, reference, or deploy any Databricks Apps resource, nor any static-website report-serving mechanism. Reports are uploaded as plain files to the Volume (step 7) — nothing more. It also MUST NOT cause any notebook copy to write into a `site/`-named path anywhere in the Volume — the project's `site/` tree (its HTML/PNG assets, published separately via GitHub Actions/Pages) is only ever regenerated by a local run against the real repo paths. Step 5's per-notebook shim table below documents every place a bootstrap cell resolves a `site/...` path and how each is redirected away from a `site`-named Volume folder.
