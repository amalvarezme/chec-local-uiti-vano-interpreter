---
description: Sube/sincroniza solo la carpeta local data/ (más el archivo site/data/variables.json que necesita el cuaderno 07, hoy archivado en old_version/) al mismo Volume de Databricks que usa /subir-a-databricks, preguntando solo la URL del workspace destino.
---
...
This is the one file this command uploads from outside `data/`, because `notebooks/project_flow/old_version/07_graph_preserved_connections_uiti_vano.ipynb` needs it as a required read-only input if it is ever run in Databricks. Since the 2026-08-04 renumbering that notebook is archived and `/subir-notebooks-databricks` no longer uploads it, so this single small file is now uploaded for completeness rather than for an active consumer — keep it, it costs nothing and removing it would silently break anyone who does upload `07` by hand. `site/` is otherwise never migrated (its HTML/PNG assets are only ever regenerated by a local run against the real repo paths — see the site-scope note in `/subir-a-databricks` and `/subir-notebooks-databricks`):
