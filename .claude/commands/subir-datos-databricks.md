---
description: Sube/sincroniza solo la carpeta local data/ (más el archivo site/data/variables.json, cuyo consumidor original era el cuaderno 07 borrado del árbol el 2026-08-14) al mismo Volume de Databricks que usa /subir-a-databricks, preguntando solo la URL del workspace destino.
---
...
This is the one file this command uploads from outside `data/`. Its original consumer was `07_graph_preserved_connections_uiti_vano.ipynb`, which read it as a required input — **that notebook was deleted from the tree on 2026-08-14, so this upload no longer has an active consumer at all.** It stays anyway, and the reason is worth stating so nobody "cleans" it twice: it is a few kilobytes, the file itself is untouched by that deletion, and the notebook is one `git checkout` away in the history. Removing the upload would turn a recoverable notebook into a notebook that comes back broken. If a future sweep retires the MGCECDL artifacts for good, this line goes with them. `site/` is otherwise never migrated (its HTML/PNG assets are only ever regenerated by a local run against the real repo paths — see the site-scope note in `/subir-a-databricks` and `/subir-notebooks-databricks`):
