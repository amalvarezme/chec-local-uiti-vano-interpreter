...
>
> Este comando publica el cuaderno `03_uiti_vano_trayectorias_circuitos.ipynb` parcheando su CODIGO por contenido:
...
>
> Ese codigo **ya no esta en el cuaderno**: vive en `src/chec_tableros/trayectorias_circuitos.py`
...
**Scope.** MUST NOT create or refresh any Delta table or view, MUST NOT touch the Lakeview dashboard, MUST NOT modify the file under `notebooks/`, MUST NOT touch the `02` app or its HTML, and MUST NOT create any `site`-named path inside the Volume.
...
Esta regla ya no tiene excepciones. **La de `04` quedo retirada el 2026-08-15**: su codigo se fue a `src/chec_tableros/trayectorias_vanos.py`, asi que su salida guardada dejo de ser fuente de nada, y la geometria K-Means que un script sacaba de ese HTML vive versionada en `data/geometria_kmeans_014_v1.json`. Ningun cuaderno del proyecto necesita conservar su salida.
