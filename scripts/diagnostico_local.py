...
`/instalar-local` podria preguntar catorce cosas con catorce lineas de shell. No lo hace
...
METAS = {
    "cuaderno": ("python", "entorno_raiz", "datos"),
    "aplicaciones": ("python", "entornos_apps", "datos", "datos_lfs", "puertos", "red"),
    "databricks": ("python", "entorno_raiz", "datos", "datos_lfs", "red",
...
def revisar_entorno_raiz() -> Revision:
    ...
    codigo, _, _ = _corre([str(py), "-c", "import torch, sklearn, pandas, plotly"], 90.0)
    if codigo != 0:
        return Revision("entorno_raiz", "Entorno de la raiz (.venv)", FALTA,
                        "existe pero le falta alguna dependencia (torch, scikit-learn, "
                        "pandas o plotly)", arreglo)
...
def revisar_entornos_apps() -> Revision:
    faltan = [a for a in APPS if not _entorno.existe(RAIZ / "aplicaciones" / a)]
    if faltan:
        return Revision("entornos_apps", "Entornos de las aplicaciones", FALTA,
                        f"les falta a {len(faltan)} de {len(APPS)}: " + ", ".join(faltan),
                        ARREGLOS["entornos_apps"])
