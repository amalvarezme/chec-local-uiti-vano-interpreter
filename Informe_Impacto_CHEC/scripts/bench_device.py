...
res = {d: medir(d) for d in ("cpu", "mps") if d == "cpu" or torch.backends.mps.is_available()}
print(f"\n--- milisegundos por lote de {BATCH} bolsas ---")
print(f"{'etapa':<28}{'CPU':>9}{'MPS':>9}")
print(f"{'1 armar el lote (Python)':<28}{t_armar:>9.2f}{t_armar:>9.2f}")
print(f"{'2 gather de X (en CPU)':<28}{t_gather:>9.2f}{t_gather:>9.2f}")
for i, e in enumerate(("transferencia","adelante","atras","sincronizar"), start=3):
    print(f"{f'{i} {e}':<28}{res['cpu'][e]:>9.2f}{res['mps'][e]:>9.2f}")
tc = t_armar + t_gather + sum(res["cpu"].values())
tm = t_armar + t_gather + sum(res["mps"].values())
print(f"{'TOTAL':<28}{tc:>9.2f}{tm:>9.2f}   ({tm/tc:.1f}x)")
med = int(np.median([len(f) for f, _ in armados]))
print(f"\ninstancias por lote (mediana): {med}  |  parametros: "
      f"{sum(p.numel() for p in construir('cpu')[0].parameters()):,}")
