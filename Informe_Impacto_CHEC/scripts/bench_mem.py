...
def correr(fijo, n=40):
    torch.mps.empty_cache()
    model, loss_fn = construir("mps")
    opt = torch.optim.AdamW(model.parameters(), lr=1e-3); model.train()
    for k in range(n):
        j = 7 if fijo else k
        lote = lotes[j]; _f, bl = armados[j]
        x = gathers[j].to("mps"); ib = torch.as_tensor(bl, dtype=torch.long, device="mps")
        y = y_cpu[lote].to("mps"); nb = nobs_cpu[lote].to("mps")
        opt.zero_grad()
        c = loss_fn.compute_components(model(x, ib, len(lote)), x, y, nb)
        c["total_loss"].backward(); opt.step()
    torch.mps.synchronize()
    return torch.mps.driver_allocated_memory() / 1e6, torch.mps.current_allocated_memory() / 1e6

for fijo, et in ((True, "forma fija   "), (False, "forma variable")):
    drv, cur = correr(fijo)
    print(f"{et}: reservado por el driver {drv:8.1f} MB | tensores vivos {cur:7.1f} MB")
