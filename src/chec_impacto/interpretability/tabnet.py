...
def construir_prompt_tabnet(context_package, skill_bundle, output_schema=None):
    ...
    return (
        ...
        "```\n\n"
        "Devuelve solo JSON valido. El reporte final genera exactamente DOS banners de discusion, "
        "uno por variable objetivo: el escenario de Numero de Eventos y el escenario de UITI_VANO. "
        "Cada banner se construye desde el campo `interpretacion` del escenario correspondiente, "
        "por lo que cada `interpretacion` debe ser independiente, autocontenida y especifica a su "
        "variable objetivo. Para cada item de `escenarios`, el campo `interpretacion` debe ser una "
        "discusion narrativa de nivel ejecutivo y operativo: un parrafo sustantivo que conecte el "
        "criterio del escenario (severidad o frecuencia), las barras, el radar, los vanos priorizados, "
        "el periodo, la lectura electrica y la cautela metodologica integrada en la redaccion. "
        "A diferencia del diagnostico base, SE TE AUTORIZA EXPLICITAMENTE a incluir inferencia "
        "predictiva, pronosticos de riesgo y proyecciones futuras de falla. "
        "No escribas listas de variables ni listas de modos dentro de `interpretacion`; "
        "usa esas senales para construir una lectura narrativa. Evita repetir limitaciones "
        "como bullets y evita titular la respuesta con el nombre de un modelo especifico. "
        "Interpreta los escenarios, sus barras de variables, sus radares por modo y la coherencia "
        "con el grafo experto sin afirmar causalidad. Lee los radares como promedio de Borda "
        "ponderado por modo normalizado min-max, no como suma total; por tanto no sobreponderan "
        "modos con muchas variables."
    )
